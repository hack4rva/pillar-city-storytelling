...
• which A City That Tells Its Stories problem your team selected
...
Where are the A City That Tells Its Stories datasets?
Show A City That Tells Its Stories problem statements
List MVP templates
...
Would you like me to analyze the A City That Tells Its Stories problem statements and suggest 3 buildable project directions?
...
The `research/` directory holds 51 deep research reports organized into sections A through I, plus framing (00–01) and cross-cutting files (90–93). Every report was generated via Perplexity `sonar-deep-research` on Richmond's civic storytelling and arts discovery problem space.

Key supporting artifacts:
- `evidence_log.md` — evidence tracker with verified claims and official URLs
- `research_notes.md` — promoted findings from verified evidence
- `03_artifacts/` — synthesized artifacts (user journeys, prototype recommendations, data landscape)
...
For arts event discovery questions: prioritize A1, A4, B1, C1, C2, D1, D3, F2, H2
For resident storytelling questions: prioritize A2, A5, B3, B4, C3, D4, F3, H3
For consent and rights questions: prioritize G2, G4
...
- For services questions: read C1 or C3 before other C files
- For data questions: read D1 before D2–D5
- For any consent or story collection question: read G2 first
...
- `(per evidence_log.md, entry E-001)`
- `[Unverified: source not read in this session]`

For statistics or specific claims: also check `evidence_log.md`. If the claim has an `E-` entry with status `Confirmed`, cite that entry.
...
|-----------|-----------------|---------|
| `research/*.md` | Yes | Factual claims about Richmond arts/storytelling |
| `evidence_log.md` | Yes | Verified specific claims with official URLs |
...
Never invent programs, datasets, services, institutions, or City positions not found in the corpus. Never claim consent requirements are met without citing G2 or G4.
